...
describe("Authentication", () => {
  test("User is able to sign up only once", async () => {
    const username = "raghav" + Math.random(); // raghav0.12331313
  ...
  test("Signup request fails if the username is empty", async () => {
    const username = `raghav-${Math.random()}`; // raghav-0.12312313
    const password = "123456";
...
describe("User avatar information", () => {
  ...
  beforeAll(async () => {
    ...
    const signupResponse = await axios.post(`${BACKEND_URL}/api/v1/sign-up`, {
      username,
      password,
      type: "admin",
    });
